/**
 * @description
 * Controller for Report on New Accounts
 */
(function (angular) {
    "use strict";
    angular.module('dataVINEApp').controller('newAccounts_ReportCtrl',
        ['$scope', '$rootScope', 'commonService', 'reportsService', '$controller', '$sessionStorage', '$templateCache',
            function ($scope, $rootScope, commonService, reportsService, $controller, $sessionStorage, $templateCache) {
                ...
                $scope.newAccounts_ReportOptions = {};
                var arrData = [];
                ...
                $scope.dynamicPopover = {
                    templateUrl: '/templates/newAccountSettings.html',
                    content: '',
                    open: function () {
                ...
                $scope.initializeTable = function (pageNum) {
                    $scope.newAccounts_ReportOptions.data = [];
                    $scope.newAccounts_ReportOptions = angular.copy(objCacheDetails.grid);
                    $scope.newAccounts_ReportOptions.exporterPdfOrientation = 'landscape',
                    $scope.newAccounts_ReportOptions.exporterPdfMaxGridWidth = 640;
                    $scope.newAccounts_ReportOptions.columnDefs = [
                        {
                            field: 'Route', displayName: 'Serial Number',
                            enableHiding: false
                        },
                        { field: 'Address', enableHiding: false },
                        { field: 'Account Name', enableHiding: false },
                        { field: 'Meter ID', displayName: 'Meter ID', enableHiding: false },
                        {
                            field: 'Instillation Date',
                            displayName: 'Installation Date', enableHiding: false
                        }];
                    $scope.newAccounts_ReportOptions.exporterSuppressColumns = ['Actions'];
                    $scope.newAccounts_ReportOptions.onRegisterApi = function (gridApi) {
                ...
                function initData(pageNum, limit) {
                    arrData.length = 0;
                    reportsService
                        .NewAccountReport(pageNum, limit, $scope.searchTerm, $scope.queryParamForAPI)
                        .then(function (apiData) {
                            if (!angular.isUndefinedOrNull(apiData) && apiData.type) {
                                ...
                                for (let i = 0; i < apiData.length; i++) {
                                    let objToInsert = {};
                                    objToInsert['Route'] = apiData[i].MeterSerialNumber;
                                    objToInsert['Address'] = apiData[i].Meters_Billing.MeterConsumerAddress;
                                    objToInsert['Account Name'] = apiData[i].Meters_Billing.MeterConsumerName;
                                    objToInsert['Meter ID'] = apiData[i].MeterID;
                                    objToInsert['Instillation Date'] = moment.tz(apiData[i].CreatedOn, userTimeZone).format('YYYY-MM-DD HH:mm:ss'); 
                                    arrData.push(objToInsert);
                                }
                                $scope.newAccounts_ReportOptions.data = arrData;
                ...
                $scope.filterRoute = function (searchTerm) {
                    if(searchTerm) {
